...
description: "Fast testing and development without API calls"
---

## Overview

During development and testing, you may want to iterate quickly without making real LLM API calls. ICICL supports mock LLM providers that return deterministic responses.
...
import pytest
import asyncio
from icicl import Agent, Trajectory
    ...
    def assert_called_with(self, expected_content: str):
        for call in self.calls:
            if expected_content in call[-1].content:
                return True
        raise AssertionError(f"'{expected_content}' not found in calls")
```
...
- [Custom LLM Providers](/guides/custom-llm-providers) - Build providers
- [LLMProvider Protocol](/api-reference/llm-provider) - Protocol definition
- [File System Agent](/examples/file-system-agent) - Full example
