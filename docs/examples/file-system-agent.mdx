...
description: "Build a complete ICICL agent that navigates a virtual file system to accomplish tasks"
---
...
async def train_agent():
    ...
    for i, task in enumerate(training_tasks, 1):
        ...
        if trajectory.success:
            print(f"✅ Success in {len(trajectory.steps)} steps")
        else:
...
async def main():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for task in training_tasks:
            ...
            print(f"Result: {'✅' if traj.success else '❌'}")
