...
await agent.train_batch(env_factory, goals, mini_batch_size=1)
await agent.run_batch(env_factory, goals, mini_batch_size=4)
agent.get_stats()
```
...
- `run_batch(..., mini_batch_size=N)` runs up to `N` inference episodes in parallel.
- `train_batch` accepts `mini_batch_size` for API symmetry, but values above `1`
  are rejected because training mutates shared database state.
