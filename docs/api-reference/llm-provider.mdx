...
The `LLMProvider` protocol defines the interface for LLM integrations. Implement this protocol to use custom LLM backends with ICRL.

```python
from icrl import LLMProvider
```
...
ICRL provides `LiteLLMProvider` which supports 100+ models:
