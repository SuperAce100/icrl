...
- PyPI project owner/maintainer access for `icrl-py`
- Python 3.12+
- `uv` installed
...
This creates distribution artifacts under `dist/` for the `icrl-py` package.
...
```bash
uvx --from ./dist/icrl_py-<version>-py3-none-any.whl \
...
- Confirm the version on PyPI: `https://pypi.org/project/icrl-py/`
- Install in a fresh environment:

```bash
pip install icrl-py==<version>
python -c "import icrl; print(icrl.__version__)"
```
