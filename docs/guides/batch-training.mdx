...
description: "Training agents on multiple tasks efficiently"
---
...
# Create factory for each task
def train_on_tasks(agent, tasks):
    trajectories = []
    for task in tasks:
...
```python
# Initial training - agent has no examples
...
<Warning>
Parallel training should be used carefully as trajectories are added to the database during training, which could cause race conditions.
</Warning>

For inference (where the database is frozen), you can parallelize:

```python
import asyncio

async def parallel_inference(agent, goals, max_concurrent=5):
    semaphore = asyncio.Semaphore(max_concurrent)
    
    async def run_one(goal):
        async with semaphore:
            env = make_env()
            return await agent.run(env, goal)
    
    tasks = [run_one(goal) for goal in goals]
    return await asyncio.gather(*tasks)

trajectories = await parallel_inference(agent, eval_goals)
```
