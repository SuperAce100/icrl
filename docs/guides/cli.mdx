...
- `tui` uses the same config resolution as `run`.
- `--model` overrides the configured model for the session.
...
Currently `chat` uses the same underlying TUI entrypoint as `tui`.
