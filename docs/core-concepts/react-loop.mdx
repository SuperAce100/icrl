---
title: ReAct Loop
description: "The Plan-Reason-Act execution loop"
---

## Overview

ICICL uses a **ReAct-style** (Reasoning + Acting) loop for agent execution. Each episode follows a structured pattern of planning, reasoning, and acting that enables systematic task completion.
...
The plan provides a strategic roadmap that guides subsequent reasoning.

<Tip>
Good plans are concise and numbered. They outline the key steps without being overly detailed.
</Tip>
...
ICICL's `ReActLoop` class encapsulates this logic:
...
loop = ReActLoop(
    llm=llm_provider,
    retriever=trajectory_retriever,
    plan_prompt="...",
    reason_prompt="...",
    act_prompt="...",
    max_steps=30,
    on_step=callback_function,
)

trajectory = await loop.run(env, goal)
```
