...
def apply() -> None:
    ...
    if getattr(DockerEnvironment, "_icrl_swebench_prebuilt_patch", False):
        return
                ...
                if getattr(self.task_env_config, "docker_image", None) is None:
                    ...
                    if dockerfile_text:
                        ...
                        if (
                            ...
                            try:
                                setattr(self, "_icrl_prebuilt_override", True)
                                setattr(self, "_icrl_prebuilt_image", base)
                            except Exception:
                                pass
            except Exception:
                # Best-effort only.
    ...
    async def patched_stop(self, delete: bool):  # type: ignore[no-untyped-def]
        ...
        if delete and getattr(self, "_icrl_prebuilt_override", False):
            image = getattr(self, "_icrl_prebuilt_image", None)
    ...
    DockerEnvironment._icrl_swebench_prebuilt_patch = True
